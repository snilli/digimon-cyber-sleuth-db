...
import got from 'got'
import fs from 'fs'
import { sleep, writeFile, readFile, existFile } from '../utils'
    ...
    try {
        for (const [key, digimon] of Object.entries(src)) {
            promise.push(
                getAndWrite(
                    digimon.detailHref,
                    `digimon/${digimon.id.padStart(3, '0')}-${
                        digimon.name
                    }.html`,
                ),
            )
    ...
    for (const file of listFile) {
            ...
            .each((trIdx, trElem) => {
                ...
                $(trElem)
                    ...
                    .each((tdIdx, tdElem) => {
                        ...
                        const id = (
                            listFile.findIndex((file) => file.includes(name)) +
                            1
                        ).toString()
        ...
        $(tableEvoInto)
            ...
            .each((trIdx, trElem) => {
                ...
                for (const [, key, value] of description.matchAll(
                    regRequirement,
                )) {
                    ...
                    if (attr === 'digimon') {
                        const attrId = (
                            listFile.findIndex((file) => file.includes(value)) +
                            1
                        ).toString()
